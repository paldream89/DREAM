...
from WriteSTORMBin_XcYcZZcFrame import write_storm_bin_XcYcZZcFrameRawZcI
from disp_angle_calculator_pickone2 import disp_angle_calculator_keepRawZcI
import time
import wx
...
for file_path in file_path_list:
    ...
    result_array_RawZc_start = np.zeros(estimated_total_num,dtype=np.float32)
    result_array_RawZc_end = np.zeros(estimated_total_num,dtype=np.float32)
    result_array_I_start = np.zeros(estimated_total_num,dtype=np.float32)
    result_array_I_end = np.zeros(estimated_total_num,dtype=np.float32)
    ...
    original_mol_RawZc = np.float32(original_mol_list['Zc'])
    original_mol_I = np.float32(original_mol_list['I'])
    ...
    while frame_cursor < total_frame and frame_index_cursor_D <= total_mol_num-1:
        ...
        if original_mol_Frame[frame_index_cursor_D]==frame_cursor:
            ...
            while original_mol_Frame[frame_index_cursor_D]==frame_cursor and frame_index_cursor_D <= total_mol_num-1:
            ...
            if frame_index_cursor_B == frame_index_cursor_C:
                ...
                correlate_num, return_array_x_start, return_array_y_start,return_array_x_end,\
                    return_array_y_end,return_array_frame, return_array_disp, return_array_angle,\
                        return_array_RawZc_start,return_array_RawZc_end,return_array_I_start,return_array_I_end\
                        = disp_angle_calculator_keepRawZcI(original_mol_Xc[frame_index_cursor_A:frame_index_cursor_B],
                                      ...
                                      original_mol_RawZc[frame_index_cursor_A:frame_index_cursor_B],
                                      original_mol_RawZc[frame_index_cursor_C:frame_index_cursor_D],
                                      original_mol_I[frame_index_cursor_A:frame_index_cursor_B],
                                      original_mol_I[frame_index_cursor_C:frame_index_cursor_D])
                ...
                result_array_angle[result_array_start_index:result_array_start_index+correlate_num] \
                    = return_array_angle
                result_array_RawZc_start[result_array_start_index:result_array_start_index+correlate_num] \
                    = return_array_RawZc_start
                result_array_RawZc_end[result_array_start_index:result_array_start_index+correlate_num] \
                    = return_array_RawZc_end
                result_array_I_start[result_array_start_index:result_array_start_index+correlate_num] \
                    = return_array_I_start
                result_array_I_end[result_array_start_index:result_array_start_index+correlate_num] \
                    = return_array_I_end
    ...
    result_array_RawZc_start_2 = np.zeros(estimated_total_num,dtype=np.float32)
    result_array_RawZc_end_2 = np.zeros(estimated_total_num,dtype=np.float32)
    result_array_I_start_2 = np.zeros(estimated_total_num,dtype=np.float32)
    result_array_I_end_2 = np.zeros(estimated_total_num,dtype=np.float32)
            ...
            if frame_index_cursor_B == frame_index_cursor_C:
                ...
                correlate_num, return_array_x_start_2, return_array_y_start_2,return_array_x_end_2,\
                    return_array_y_end_2,return_array_frame_2, return_array_disp_2, return_array_angle_2,\
                        return_array_RawZc_start_2,return_array_RawZc_end_2,return_array_I_start_2,return_array_I_end_2\
                        = disp_angle_calculator_keepRawZcI(original_mol_Xc[frame_index_cursor_A:frame_index_cursor_B],
                                      original_mol_Yc[frame_index_cursor_A:frame_index_cursor_B],
                                      original_mol_Xc[frame_index_cursor_C:frame_index_cursor_D],
                                      original_mol_Yc[frame_index_cursor_C:frame_index_cursor_D],
                ...
                result_array_RawZc_start_2[result_array_start_index_2:result_array_start_index_2+correlate_num] \
                    = return_array_RawZc_start_2
                result_array_RawZc_end_2[result_array_start_index_2:result_array_start_index_2+correlate_num] \
                    = return_array_RawZc_end_2
                result_array_I_start_2[result_array_start_index_2:result_array_start_index_2+correlate_num] \
                    = return_array_I_start_2
                result_array_I_end_2[result_array_start_index_2:result_array_start_index_2+correlate_num] \
                    = return_array_I_end_2
    ...
    save_file_path = file_path.replace('.bin','-keepZcI-dis'+str(disp_thre)+'.bin')
    
    write_storm_bin_XcYcZZcFrameRawZcI(save_file_path, total_frame, result_array_start_index+result_array_start_index_2, 
                                 ...
                                 np.hstack((result_array_RawZc_start[0:result_array_start_index],result_array_RawZc_start_2[0:result_array_start_index_2])),
                                 np.hstack((result_array_RawZc_end[0:result_array_start_index],result_array_RawZc_end_2[0:result_array_start_index_2])),
                                 np.hstack((result_array_I_start[0:result_array_start_index],result_array_I_start_2[0:result_array_start_index_2])),
                                 np.hstack((result_array_I_end[0:result_array_start_index],result_array_I_end_2[0:result_array_start_index_2])))
