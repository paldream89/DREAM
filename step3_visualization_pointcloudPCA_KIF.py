...
time_interval = 0.05  # unit is s, so 1 ms
...
def onSelect(x):
    ...
    selected_STORMnp = STORM_npdata['Z'][ind] # if 2D, angle=100 
    ...
    guessed_d = np.average(bins_x_1D,weights=selected_hist_sum_1D)
    ...
    try: 
        ...
        popt,_ = optimize.curve_fit(oneD_kinesin_dis_func,bins_x_1D,selected_hist_sum_1D,p0=p0,bounds=bounds)
        selected_drate = abs(popt[3] * pixel_size / time_interval)
        ...
        fitted_y = popt[2]*np.exp(-(smooth_x-popt[3])**2/popt[0])+popt[1]
        # c*np.exp(-(x-d)**2/a)+b
        # distribution_y = 2*popt[2]*smooth_x/popt[0]*np.exp(-smooth_x**2/popt[0])
        # background_y = popt[1]*smooth_x
        plt.plot(smooth_x, fitted_y,'k')
