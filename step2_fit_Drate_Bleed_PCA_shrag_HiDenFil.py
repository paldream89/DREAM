...
from hist_maker_PCA_shrinkbinrange import hist_maker_bleed_HighDen
...
if __name__=="__main__":
    ...
    pixel_size = 167 # unit is nm
    image_bin_size = 0.8 # unit is pixel
    ...
    mode_trigger = 3
    disp_thre = 10
    time_interval = 0.0005 # unit is s, so 1 ms
    
    # histogram binning setting
    hist_bin_num = 2*disp_thre # must be an interger
    ...
    for file_path in file_path_list:
        ...
        with Pool(processes = cpu_used) as pool:
            averaged_hist_array = pool.map(partial(hist_maker_bleed_HighDen,disp_thre=disp_thre,
                                                  ...
                                                  HighDen_thre=HighDen_thre,
                                                  frame_num=frame_num),STORM_npdata)
            pool.close()
        ...
        save_file_path = file_path.replace('.bin','-avePCABd'+str(true_bin_size)+'nm'+'.bin')
