...
import numpy as np
from ReadSTORMBin_XcYcZZcFrame import read_storm_bin_XcYcFramedisAngledisxy
...
from hist_maker_PCA_shrinkbinrange import hist_maker_Z_Bleed
from drate_fitter_PCA_shrinkbinrange import drate_fitter_Z
...
if __name__=="__main__":
    ...
    data_type = np.dtype([('x_inter', np.int16),('y_inter', np.int16),('frame', np.int32),
                          ...
                          ('xc', np.float32), ('yc', np.float32), ('disp_xy', np.float32)])
    ...
    for file_path in file_path_list:
        ...
        # read the data
        frame_num, number_of_points, STORM_npdata = read_storm_bin_XcYcFramedisAngledisxy(file_path)
        ...
        xinter_yinter_frame_disp_angle_xcyc['angle'] = np.tile(STORM_npdata['angle'], \
                                                            mode_trigger+2)
        xinter_yinter_frame_disp_angle_xcyc['disp_xy'] = np.tile(STORM_npdata['disp_xy'], \
                                                            mode_trigger+2)
        ...
        with Pool(processes = int(cpu_count()/cpu_ratio)) as pool:
            averaged_hist_array = pool.map(partial(hist_maker_Z_Bleed,disp_thre=disp_thre,
        ...
        with Pool(processes = int(cpu_count()/cpu_ratio)) as pool:
            averaged_drate = pool.map(partial(drate_fitter_Z,disp_thre=disp_thre,
                                                   hist_bin_num=hist_bin_num,
        ...
        save_file_path = file_path.replace('.bin','-avePCA'+str(true_bin_size)+'nm'+'Z.bin')
