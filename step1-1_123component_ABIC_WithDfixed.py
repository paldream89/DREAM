...
time_interval = 0.006896# unit is s, so 1 ms
component = 2 # 1 or 2 or 3 component? Use 23 to fit 2-component and 3-component models together.
...
# -------------------------------------------------------------------------
# The image shown for lasso selection uses STORM_npdata_D as the visual template.
# Its y coordinate is flipped below. Therefore, the coordinates used for
# selecting STORM_npdata must be transformed into the same display coordinate
# system. Otherwise, the lasso polygon and STORM_npdata points are compared in
# different coordinate systems, giving apparently random/mismatched selection.
y_max = np.amax(STORM_npdata_D['Yc'])
STORM_npdata_D['Yc'] = y_max-STORM_npdata_D['Yc']
x_max = np.amax(STORM_npdata_D['Xc'])

# Coordinates of the displacement points in the same display coordinate system
# as the template image. Do not rely on index correspondence between
# STORM_npdata and STORM_npdata_D; only the spatial coordinates are used.
...
# Lasso selection is performed on the displayed template axes.
# Therefore pix must use the transformed display coordinates of STORM_npdata.
