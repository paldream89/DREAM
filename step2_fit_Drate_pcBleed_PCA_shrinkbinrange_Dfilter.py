...
if __name__=="__main__":
    ...
    # Diffusion rate filter
    D_filter_lower = 2.5 # only keep points with D>D_filter_lower
    ...
    for file_path in file_path_list:
        ...
        remove_unuseful_indice = averaged_drate_stack > D_filter_lower
        ...
        save_file_path = file_path.replace('.bin','-avePCABd'+str(true_bin_size)+'nm'+'Df'+str(D_filter_lower)+'.bin')
    ...
    #                ('Z', np.float32), ('Zc', np.float32)])
        ...
        save_file_hist_map = file_path.replace('.bin','-HistPCABd'+'-Add'+str(mode_trigger)+\
                                                   'pt-'+'bsize'+str(true_bin_size)+'nm'+\
                                                        '-bnum'+str(hist_bin_num)+'Df'+str(D_filter_lower)+'.npy')
