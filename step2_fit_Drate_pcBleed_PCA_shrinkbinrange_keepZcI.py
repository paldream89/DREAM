...
import numpy as np
from ReadSTORMBin_XcYcZZcFrame import read_storm_bin_XcYcZZcFrame_RawZcI
...
from hist_maker_PCA_shrinkbinrange import hist_maker_bleed_keepZcI
from drate_fitter_PCA_shrinkbinrange import drate_fitter_keepZcI
...
if __name__=="__main__":
    ...
    openFileDialog = wx.FileDialog(frame, "Select Multiple Bin with dis-keepZcI files", "", "", 
          "Bin files (*.bin)|*.bin", 
    ...
    for file_path in file_path_list:
        ...
        # read the data
        frame_num, number_of_points, STORM_npdata = read_storm_bin_XcYcZZcFrame_RawZcI(file_path)
        ...
        xinter_yinter_frame_disp_angle_xcyc['RawZc'] = \
            ((np.dot(temp_index2,np.stack((STORM_npdata['RawZc_start'],STORM_npdata['RawZc_end']),axis=0))\
              .reshape(-1,1).squeeze(axis=1)))
        xinter_yinter_frame_disp_angle_xcyc['I'] = \
            ((np.dot(temp_index2,np.stack((STORM_npdata['I_start'],STORM_npdata['I_end']),axis=0))\
              .reshape(-1,1).squeeze(axis=1)))
        ...
        with Pool(processes = int(cpu_count()/cpu_ratio)) as pool:
            averaged_hist_array = pool.map(partial(hist_maker_bleed_keepZcI,disp_thre=disp_thre,
        ...
        with Pool(processes = int(cpu_count()/cpu_ratio)) as pool:
            averaged_drate = pool.map(partial(drate_fitter_keepZcI,disp_thre=disp_thre,
                                                   hist_bin_num=hist_bin_num,
        ...
        Write_STORMbin(save_file_path, np.int32(useful_number_of_points), 1000,
                        ...
                        (np.hstack((averaged_hist_array[:,10+2*hist_bin_num].astype(np.float32),averaged_hist_array[:,11+2*hist_bin_num].astype(np.float32))))[remove_unuseful_indice], 
                        # this is RawZc in BG
                        (np.hstack((averaged_hist_array[:,12+2*hist_bin_num].astype(np.float32),averaged_hist_array[:,13+2*hist_bin_num].astype(np.float32))))[remove_unuseful_indice], 
                        # this is I Intensity
