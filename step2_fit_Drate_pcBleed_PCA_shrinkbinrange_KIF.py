...
from hist_maker_PCA_shrinkbinrange import hist_maker_bleed
from drate_fitter_PCA_shrinkbinrange import drate_fitter_kinesin
...
if __name__=="__main__":
    ...
    image_bin_size = 0.9 # unit is pixel
    ...
    mode_trigger = 1
    disp_thre = 5
    time_interval = 0.05  # unit is s, so 50 ms
    
    # histogram binning setting
    hist_bin_num = 6*disp_thre # must be an interger
    ...
    # pca analysis threshold, important for directional motion, e.g. kinesin
    pca_ratio_thre = 0.7
    ...
    for file_path in file_path_list:
        ...
        with Pool(processes = cpu_count()) as pool:
            averaged_hist_array = pool.map(partial(hist_maker_bleed,disp_thre=disp_thre,
        ...
        with Pool(processes = cpu_count()) as pool:
            averaged_drate = pool.map(partial(drate_fitter_kinesin,disp_thre=disp_thre,
                                                   hist_bin_num=hist_bin_num,
        ...
        category_array[z_array_angle > 10] = 1 # because if 2D, manually set angle=100
        save_file_path = file_path.replace('.bin','-avePCABd'+str(true_bin_size)+'nm'+'-KIF'+'.bin')
