...
time_interval = 0.0065359 # unit is s, so 1 ms
D = 2.0 # unit is um2/s
Var_D = 0.4 # unit is um2/s
D2 = 1.0 # unit is um2/s
Var_D2 = 0.2 # unit is um2/s
D3 = 0.2 # unit is um2/s
Var_D3 = 0.1 # unit is um2/s
# a = D*4*time_interval/(pixel_size/1000)/(pixel_size/1000)
...
def onSelect(x):
    ...
    while cursor_B < size_selected:
        ...
        guessed_a = D*4*time_interval/(pixel_size/1000)/(pixel_size/1000)
        guessed_b = (hist[-1]/bins_x[-1] + hist[-2]/bins_x[-2])/2
        guessed_c = np.amax(hist[0:bin_thre])*sqrt(guessed_a)*e/2
        guessed_a2 = D2*4*time_interval/(pixel_size/1000)/(pixel_size/1000)
        guessed_c2 = np.amax(hist[0:bin_thre])*sqrt(guessed_a)*e/2
        guessed_a3 = D3*4*time_interval/(pixel_size/1000)/(pixel_size/1000)
        guessed_c3 = np.amax(hist[0:bin_thre])*sqrt(guessed_a)*e/10
        var_a = Var_D*4*time_interval/(pixel_size/1000)/(pixel_size/1000)
        var_a2 = Var_D2*4*time_interval/(pixel_size/1000)/(pixel_size/1000)
        var_a3 = Var_D3*4*time_interval/(pixel_size/1000)/(pixel_size/1000)
                
        p0 = [guessed_a,guessed_b,guessed_c,guessed_a2,guessed_c2,guessed_a3,guessed_c3]
        bounds = ([guessed_a-var_a,0,0,guessed_a2-var_a2,0,guessed_a3-var_a3,0],
                  [guessed_a+var_a,np.inf,np.inf,guessed_a2+var_a2,np.inf,guessed_a3+var_a3,np.inf])
        ...
        try: 
            ...
            popt,_ = optimize.curve_fit(twoD_diff_dis_func_triple,bins_x,hist,p0=p0,bounds=bounds)
            ...
            print('Selected Area D-2D-1 first D: '+str(drate))
            print('Selected Area D-2D-2 second D: '+str(drate_2))
            print('Selected Area D-2D-3 third D: '+str(drate_3))
            print('Selected Area b-2D: '+str(slope))
            ...
            fitted_y = 2*popt[2]*smooth_x/popt[0]*np.exp(-smooth_x**2/popt[0])+popt[1]*smooth_x+\
                2*popt[4]*smooth_x/popt[3]*np.exp(-smooth_x**2/popt[3])+\
                    2*popt[6]*smooth_x/popt[5]*np.exp(-smooth_x**2/popt[3])
            ...
            distribution_y3 = 2*popt[6]*smooth_x/popt[5]*np.exp(-smooth_x**2/popt[3])
            background_y = popt[1]*smooth_x
            print('D1 first D ratio: '+str(np.sum(distribution_y)/(np.sum(distribution_y2)+np.sum(distribution_y)+np.sum(distribution_y3))))
            print('D2 second D ratio: '+str(np.sum(distribution_y2)/(np.sum(distribution_y2)+np.sum(distribution_y)+np.sum(distribution_y3))))
            print('D3 third D ratio: '+str(np.sum(distribution_y3)/(np.sum(distribution_y2)+np.sum(distribution_y)+np.sum(distribution_y3))))
            plt.plot(smooth_x, fitted_y,'k')
            plt.plot(smooth_x, distribution_y,'r--')
            plt.plot(smooth_x, distribution_y2,'m--')
            plt.plot(smooth_x, distribution_y3,'y--')
