...
time_interval = 0.006135 # unit is s, so 1 ms
component = 2 # 1 or 2 component?
# point cloud point size
point_size = 0.01

# define the two Knonwn D
D_slow = 0.04
D_fast = 4
...
toleration_slow = 0.001 # D_slow +- toleration_slow, for both 1D and 2D
toleration_fast = 4 # D_fast +- toleration_fast, for both 1D and 2D
...
def onSelect(x):
    ...
    while cursor_B < size_selected:
        ...
        a = D_slow*4*time_interval/((pixel_size/1000)**2)
        a2 = D_fast*4*time_interval/((pixel_size/1000)**2)
        
        a_lower = (D_slow-toleration_slow)*4*time_interval/((pixel_size/1000)**2)
        a_upper = (D_slow+toleration_slow)*4*time_interval/((pixel_size/1000)**2)
        a2_lower = (D_fast-toleration_fast)*4*time_interval/((pixel_size/1000)**2)
        a2_upper = (D_fast+toleration_fast)*4*time_interval/((pixel_size/1000)**2)
        
        a_1D = D_slow_1D*4*time_interval/((pixel_size/1000)**2)
        a2_1D = D_fast_1D*4*time_interval/((pixel_size/1000)**2)
        
        a_1D_lower = (D_slow_1D-toleration_slow)*4*time_interval/((pixel_size/1000)**2)
        a_1D_upper = (D_slow_1D+toleration_slow)*4*time_interval/((pixel_size/1000)**2)
        a2_1D_lower = (D_fast_1D-toleration_fast)*4*time_interval/((pixel_size/1000)**2)
        a2_1D_upper = (D_fast_1D+toleration_fast)*4*time_interval/((pixel_size/1000)**2)
        
        guessed_a = a
        guessed_b = (hist[-1]/bins_x[-1] + hist[-2]/bins_x[-2])/2
        guessed_c = np.amax(hist[0:bin_thre])*sqrt(a)*e/2
        guessed_a2 = a2
        guessed_c2 = np.amax(hist[0:bin_thre])*sqrt(a2)*e/2
                
        p0 = [guessed_a,guessed_b,guessed_c,guessed_a2,guessed_c2]
        bounds = ([a_lower,0,0,a2_lower,0],[a_upper,np.inf,np.inf,a2_upper,np.inf])
