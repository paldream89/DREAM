...
time_interval = 0.004525  # unit is s, so 1 ms
...
toleration_slow = 0.01 # D_slow +- toleration_slow, for both 1D and 2D
toleration_fast = 0.01 # D_fast +- toleration_fast, for both 1D and 2D
...
def onSelect(x):
    ...
    if component == 1:
        ...
        p0 = [guessed_a,guessed_b,guessed_c,guessed_a2,guessed_c2]
        bounds = ([a_1D_lower,0,0,a2_1D_lower,0],[a_1D_upper,np.inf,np.inf,a2_1D_upper,np.inf])
        
        try:
            
            popt,_ = optimize.curve_fit(oneD_diff_dis_func_double,bins_x_1D,selected_hist_sum_1D,p0=p0,bounds=bounds)
            drate = popt[0] * (pixel_size/1000)**2 /4/time_interval
            drate_2 = popt[3] * (pixel_size/1000)**2 /4/time_interval
