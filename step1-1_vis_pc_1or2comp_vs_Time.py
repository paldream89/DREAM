...
time_interval = 0.005952 # unit is s, so 1 ms
component = 2 # 1 or 2 component?
# point cloud point size
point_size = 0.1
lock_D_slow = 0.045 # unit is um2/s
lock_D_slow_error = 0.005

# frame to mins
camera_fre = 168 # unit is Hz
total_frame = 480000
time_max = total_frame/camera_fre/60 # unit is min

# histogram binning setting
bin_num = disp_thre*5 # must be an interger
...
angle_bin_num = 10  # from -pi to pi
angle_bin_size = total_frame/angle_bin_num
angle_bins_x = np.arange(0.5*angle_bin_size, total_frame, angle_bin_size, dtype=np.int32)

angle_bin_size_plot = angle_bin_size/camera_fre/60
angle_bins_x_plot = np.arange(0.5*angle_bin_size_plot, 
                              time_max, angle_bin_size_plot, dtype=np.float32)
...
def onSelect(x):
    ...
    selected_angle = STORM_npdata['frame'][ind]
    plt.figure()
    cmap = mpl.colormaps["jet"]
    cmap.set_under(color='black')
    cmap.set_bad(color='black')
    ...
    while cursor_B < size_selected:
        ...
        while selected_angle[cursor_B]<cursor_angle*angle_bin_size:
        ...
        if component == 1:
            ...
            try: 
                ...
                guessed_b = (hist[-1]/bins_x[-1] + hist[-2]/bins_x[-2])/2
                guessed_c = np.amax(hist[0:bin_thre])*sqrt(guessed_a)*e/2
                guessed_a2 = lock_D_slow*time_interval*4/((pixel_size/1000)**2)
                guessed_c2 = np.amax(hist[0:bin_thre])*sqrt(guessed_a)*e/2
                
                guessed_a2_upper = (lock_D_slow+lock_D_slow_error)*time_interval*4/((pixel_size/1000)**2)
                guessed_a2_lower = (lock_D_slow-lock_D_slow_error)*time_interval*4/((pixel_size/1000)**2)
                        
                p0 = [guessed_a,guessed_b,guessed_c,guessed_a2,guessed_c2]
                bounds = ([0,0,0,guessed_a2_lower,0],[disp_thre**2,np.inf,np.inf,guessed_a2_upper,np.inf])
